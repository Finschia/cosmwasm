...
use cosmwasm_std::{
    entry_point, from_slice, to_binary, to_vec, Addr, AllBalanceResponse, Api, Binary,
    CanonicalAddr, CosmosMsg, Deps, DepsMut, Env, MessageInfo, QueryRequest, QueryResponse,
...
pub fn sudo(_deps: DepsMut, env: Env, msg: SudoMsg) -> Result<Response, HackError> {
    match msg {
        SudoMsg::StealFunds { recipient, amount } => {
            ...
            response.add_message(CosmosMsg::Stargate {
...
fn do_release(deps: DepsMut, env: Env, info: MessageInfo) -> Result<Response, HackError> {
    ...
    if info.sender == state.verifier {
        ...
        resp.add_message(CosmosMsg::Stargate {
            type_url: "/lfb.bank.v1beta1.MsgSend".into(),
            value: Binary::encode_prost_message(&stargate_msg)?,
        });
