...
/// Create ostracon.rs with library information
pub fn generate_ostracon_lib(prost_dir: &PathBuf, ostracon_lib_target: &PathBuf) {
    let file_names = WalkDir::new(prost_dir)
        .into_iter()
        .filter_map(|e| e.ok())
        .filter(|e| {
            e.file_type().is_file()
                && e.file_name().to_str().unwrap().starts_with("ostracon.")
    ...
    let mut content =
        String::from("//! Ostracon-proto auto-generated sub-modules for Ostracon\n");
    let tab = "    ".to_string();

    for file_name in file_names {
        let parts: Vec<_> = file_name
            .strip_prefix("ostracon.")
            .unwrap()
            .strip_suffix(".rs")
            .unwrap()
            .split('.')
    ...
    content = format!(
        ...
        content,
        tab,
        crate::constants::OSTRACON_REPO,
        tab,
        crate::constants::OSTRACON_COMMITISH,
    );

    let mut file =
        File::create(ostracon_lib_target).expect("ostracon library file create failed");
    file.write_all(content.as_bytes())
        .expect("ostracon library file write failed");
}
